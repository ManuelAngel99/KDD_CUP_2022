...
def is_valid_prediction(prediction, idx=None):
    ...
    try:
        if prediction.ndim > 1:
            nan_prediction = np.isnan(prediction).any(axis=1)
...
def mae(pred, gt, run_id=0):
    ...
    if is_valid_prediction(pred, idx=run_id):
        if pred.shape != gt.shape:
            raise MetricsError(
                "Different shapes between Prediction ({}) and Ground Truth ({}) "
...
def turbine_scores(prediction, targets, examine_len, idx=0):
    """
    Desc:
    ...
    Args:
        ...
        idx:
    Returns:
        The averaged MAE and RMSE and a vector containing the indices of the valids predictions.
    """
    ...
    if np.any(prediction) and np.any(targets):
        _mae, _rmse = regressor_scores(
            prediction[-examine_len:] / 1000, targets[-examine_len:] / 1000, idx=idx
        )
    return _mae, _rmse
...
def is_zero_prediction(predictions, identifier, settings):
    ...
    if non_zero_ratio < settings["min_non_zero_ratio"]:
        msg = "{:.2f}% turbines with zero predicted values " "(in {}th prediction)!".format(
            (1 - non_zero_ratio) * 100, identifier
        )
        raise MetricsError(msg)
    return False
    ...
    try:
        if np.min(prediction) == np.max(prediction):
            if idx is None:
                ...
                msg = "All predicted values are same as {:.4f} ({}th prediction)!".format(
                    np.min(prediction), idx
                )
            logger.debug(msg)
            return -1
        if np.std(prediction) <= min_std:
            ...
            if distinct_ratio < min_distinct_ratio:
                if idx is None:
                    msg = (
                        "{:.2f}% of predicted values are same! Some predicted values are: "
                        "{},...".format((1 - distinct_ratio) * 100, samples)
                    )
                else:
                    msg = (
                        "{:.2f}% of predicted values are same ({}th run)! "
...
def is_identical_prediction(predictions, identifier, settings):
    ...
    for i in range(settings["capacity"]):
        prediction = predictions[i]
        status = check_identical_prediction(
            prediction,
            min_distinct_ratio=settings["min_distinct_ratio"],
            idx=identifier,
        )
    ...
    if variation_ratio < settings["min_distinct_ratio"] and not settings["is_debug"]:
        msg = (
            "{:.2f}% turbines with (almost) identical predicted values "
...
def regressor_detailed_scores(predictions, gts, raw_df_lst, settings):
    ...
    if (
        not is_identical_prediction(predictions, identifier, settings)
        and not is_zero_prediction(predictions, identifier, settings)
        or settings["is_debug"]
    ):
        pass
    for i in range(settings["capacity"]):
        raw_df = raw_df_lst[i]
        ...
        valid_indices = raw_df["Abnormal"] == False
        all_valid_indices.append(valid_indices)

        if not is_valid_prediction(prediction, idx=i):
            continue
        ...
        if (
            -1 == _mae or -1 == _rmse
        ):  # In case the target is empty after filtering out the abnormal values
            continue
        all_mae.append(_mae)
        all_rmse.append(_rmse)
        latest_mae, latest_rmse = turbine_scores(
            prediction,
            gt,
            settings["day_len"],
            idx=identifier,
    ...
    if total_mae < 0 or total_rmse < 0:
        raise MetricsError(
            "{}th prediction: summed MAE ({:.2f}) or RMSE ({:.2f}) is negative, "
    ...
    if len(all_mae) == 0 or len(all_rmse) == 0 or total_mae == 0 or total_rmse == 0:
        raise MetricsError(
            "No valid MAE or RMSE for "
            "all of the turbines in {}th prediction! ".format(identifier)
        )
    total_latest_mae = np.array(all_latest_mae, dtype=np.float32).sum()
    total_latest_rmse = np.array(all_latest_rmse, dtype=np.float32).sum()
    all_valid_indices = np.array(all_valid_indices)
    return total_mae, total_rmse, total_latest_mae, total_latest_rmse, all_valid_indices
